...
def show_message_body(queue):
	msgs = queue.list(include_claimed=True, echo=True, limit=1)
	if len(msgs) > 0:
		print msgs[0].body

def main():
	p = optparse.OptionParser(description="Cloud Queues Demo Message Display",
							  prog="python show_first_message.py",
							  version="0.1",
							  usage="%prog [options]")

	p.add_option('--region', '-r',
				 default='IAD',
				 help="The Region in which to show messages")
